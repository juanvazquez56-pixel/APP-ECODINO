...
function checklistPct(responses: Record<string, boolean> | undefined): number {
  ...
  return Math.round((n / SUPERVISOR_CHECKLIST_TOTAL) * 100);
}

export function SupervisorHome() {
  const navigate = useNavigate();
  const { profile, signOut } = useAuth();
  const newDraft = useSupervisorStore((s) => s.newDraft);

  const reports = useLiveQuery(
    () => db.supervisorReports.orderBy('lastModified').reverse().limit(10).toArray(),
    [],
    [],
  );
  ...
  const handleNew = () => {
    const id = newDraft();
    navigate(`/supervisor/reporte/${id}`);
  };
      ...
      <main className="flex flex-1 flex-col gap-4 p-4">
        ...
        {todayReport ? (
          <Card variant={todayReport.data?.draft?.status === 'borrador' ? 'warn' : 'success'}>
            <p className="text-sm text-slate-500">Reporte de hoy</p>
            ...
            <Button
              className="mt-3"
              size="sm"
              onClick={() => navigate(`/supervisor/reporte/${todayReport.localId}`)}
            >
        ...
        {!reports || reports.length === 0 ? (
          <EmptyState
            ...
            description="Crea tu primer reporte del día con el botón de arriba."
          />
        ) : (
          <div className="flex flex-col gap-2">
            {reports.map((r) => {
              ...
              return (
                <button
                  key={r.localId}
                  onClick={() => navigate(`/supervisor/reporte/${r.localId}`)}
